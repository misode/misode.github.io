...
function GeneratorTitle({ title, gen }: GeneratorTitleProps) {
	...
	const getGenerators = useCallback(
		(search: string, close: () => void) => {
			...
			return results.map((g) => (
				<Link
					class='flex items-center cursor-pointer no-underline rounded p-1'
					href={cleanUrl(g.url)}
					onClick={close}
				>
	...
	return (
		<FancyMenu
			getResults={getGenerators}
			placeholder={locale('generators.search')}
		>
